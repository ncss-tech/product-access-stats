...
load('processed-data/app-1x.Rda')

# note that there is a gap
x <- subset(x.daily, subset=date > as.Date('2012-06-01'))

# init TS object
x.ts <- ts(x$freq, frequency = 30, start=c(2012, 6, 1))
...
filename <- paste0('figures/app-1x_daily-ts-decomposition.png')
